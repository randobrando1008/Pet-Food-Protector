...
void Open(void)
{
    ...
    while (1)
    {
        ...
        if (k>=1)            //Change motor position every period (100 times)
        {
            ...
            k=0;             // Reset Increment every period
        }
...
void Close(void)
{
    ...
    OC1R=OC1RS=2800;         //Vertical position, 7% Duty Cycle (2800/40000)
    OC1CON=0x000E;           //Continue in Idle, Select Timer 3, PWM Mode 
    ...
    while (1)
    {
        ...
        if (k>=1)            //Change motor position every period (100 times)
        {
            OC1RS=OC1RS-12;  //Decrease duty cycle for PWM to change position
            if (OC1RS<=1600) //Horizontal position, 4% Duty Cycle (1600/40000)
            {
                return 1;    //Stop once the lid is closed
            }
            k=0;             // Reset Increment every period
        }
